...
def get_teacher_courses(current_user):
    ...
    if current_user.role != 'teacher' and current_user.role != 'admin':
        logger.warning(f"Unauthorized access to teacher courses by user {current_user.id} with role {current_user.role}")
        return jsonify({'message': 'You do not have permission to access teacher courses'}), 403
